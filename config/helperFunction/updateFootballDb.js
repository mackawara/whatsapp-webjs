...
const updateFixtures = async ({ league, fixtureIDs }) => {
  ...
  try {
    results.forEach(async result => {
      ...
      const queryFixturesAndSave = async function () {
        ...
        if (!result) {
          await fixture
            .save()
            .then(result => logger.silly(`${result.fixture} saved`));
        } else {
          result.overwrite({
            matchStatus: matchStatus,
            fixture: `${home} vs ${away}`,
